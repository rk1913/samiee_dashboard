...
export async function syncGitHubContributions(username: string): Promise<{ success: boolean; count: number; error?: string }> {
  ...
  try {
    ...
    const changedDays = daysToUpsert.filter(day => {
      ...
      return existing.githubCount !== day.count || existing.githubContributed !== (day.count > 0);
    });

    console.log(`Only ${changedDays.length} of ${daysToUpsert.length} days of GitHub contributions actually changed and need syncing.`);
